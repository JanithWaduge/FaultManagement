...
import PhotoUploadForm from "./PhotoUploadForm"; // Import the PhotoUploadForm component
import { PhotoModal } from "./components/PhotoModal"; // Import the updated PhotoModal
import { useMultiFaults } from "./useMultiFaults";

const assignablePersons = ["John Doe", "Jane Smith", "Alex Johnson", "Emily Davis"];

// PhotoModal component to display photos (already updated above)
function FaultsTable({ faults, onEdit, onMarkResolved, isResolved, page, setPage, max, onOpenEditModal, onOpenNotesModal }) {
  ...
  const [uploadModalOpen, setUploadModalOpen] = useState(null); // Track faultId for upload modal
  ...
  const handleUploadSuccess = (faultId) => {
    setUploadModalOpen(null); // Close upload modal
          ...
          <Card.Body className="p-3 d-flex flex-column">
            ...
            <nav className="mt-3 px-3" style={{ flexShrink: 0 }}>
              <ul className="pagination justify-content-center mb-0">
                <li className={`page-item ${page === 1 ? "disabled" : ""}`}><button className="page-link" onClick={() => setPage(Math.max(page - 1, 1))}>Previous</button></li>
                {Array.from({ length: max }).map((_, idx) => <li key={idx + 1} className={`page-item ${page === idx + 1 ? "active" : ""}`}><button className="page-link" onClick={() => setPage(idx + 1)}>{idx + 1}</button></li>)}
                <li className={`page-item ${page === max || max === 0 ? "disabled" : ""}`}><button className="page-link" onClick={() => setPage(Math.min(page + 1, max))}>Next</button></li>
              </ul>
            </nav>
          </Card.Body>
        </Card>
...
export default function Dashboard({ userInfo, notifications, setNotifications, onLogout }) {
  ...
  useEffect(() => {
    ...
    const outside = (e) => { if (notifRef.current && !notifRef.current.contains(e.target)) setShowNotif(false); };
  ...
  const filtered = useMemo(() => sortedFaults.filter(f => f && [f.DescFault, f.Location, f.LocationOfFault, f.ReportedBy, f.SystemID].join(" ").toLowerCase().includes(search.toLowerCase())), [sortedFaults, search]);
  ...
  return (
    <>
      ...
      <footer className="fixed-bottom text-white py-2 px-3 d-flex flex-column flex-sm-row justify-content-between align-items-center shadow" style={{ backgroundColor: "#001f3f" }}>
        <div className="mb-2 mb-sm-0"><Button className="glass-button" size="sm" onClick={() => alert("Contact support at support@nfm.lk")}>Support</Button></div>
        <div className="text-center flex-grow-1 mb-2 mb-sm-0">Total Open: {open.length} | Resolved: {resolved.length} | Unread Notifications: {notifications.filter(n => !n.isRead).length}</div>
        <div className="text-center text-sm-end">
          <Button className="glass-button" size="sm" onClick={() => setFooterInfo(v => !v)}>{footerInfo ? "Hide Info" : "Show Info"}</Button>
          {footerInfo && <div className="mt-1" style={{ fontSize: "0.75rem", opacity: 0.8 }}>© 2025 Network Fault Management System. All rights reserved.</div>}
        </div>
      </footer>
