...
router.get("/locations", authenticateToken, async (req, res) => {
  try {
    const result = await db.query(
      "SELECT * FROM tblFaultLoc ORDER BY LocaFault"
    );
    res.json(result.recordset);
  } catch (error) {
...
router.get("/sections", authenticateToken, async (req, res) => {
  try {
    const result = await db.query(
      "SELECT * FROM tblSections ORDER BY section_name"
    );
    res.json(result.recordset);
  } catch (error) {
...
router.post(
  ...
  [
    authenticateToken,
    body("name").trim().notEmpty().withMessage("System name is required"),
  ],
  async (req, res) => {
    try {
      ...
      const result = await db.query(
        "INSERT INTO tblSystem (System) VALUES (@systemName); SELECT SCOPE_IDENTITY() as id;",
        { systemName: name }
      );

      const systemId = result.recordset[0].id;

      // Return the new system in the expected format
      res.status(201).json({
        id: systemId,
        System: name,
      });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await db.query(
        "INSERT INTO tblFaultLoc (LocaFault) VALUES (@locationName); SELECT SCOPE_IDENTITY() as id;",
        { locationName: name }
      );

      const locationId = result.recordset[0].id;

      // Return the new fault location in the expected format
      res.status(201).json({
        id: locationId,
        LocaFault: name,
        FaultId: null,
      ...
      res.status(500).json({ message: "Failed to add fault location" });
    }
  }
);
...
router.get("/dashboard/stats", authenticateToken, async (req, res) => {
  try {
    const stats = await Promise.all([
      ...
      db.query(
        "SELECT COUNT(*) as inProgress FROM tblFaults WHERE Status = 'In Progress'"
      ),
      db.query(
        "SELECT COUNT(*) as resolved FROM tblFaults WHERE Status = 'Resolved'"
      ),
      db.query(
        "SELECT COUNT(*) as closed FROM tblFaults WHERE Status = 'Closed'"
      ),
    ]);
